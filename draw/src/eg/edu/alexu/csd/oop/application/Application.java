...
public class Application extends JFrame implements ActionListener, MouseListener, MouseMotionListener{
	...
	private Map<String, Double> getMap(Shape shape, boolean zoomIn, boolean zoomOut){
		...
		Object[] y = drawPanel.drawingEngine.getShapes()[shapeIndex].getProperties().values().toArray();
		int x = 0;
		if(zoomIn == true){
			for(String g : drawPanel.drawingEngine.getShapes()[shapeIndex].getProperties().keySet()){
	...
	private Shape getReplaceableShape(Shape shape, boolean zoomIn, boolean zoomOut,
